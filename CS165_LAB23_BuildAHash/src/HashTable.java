...
public class HashTable {
    ...
    public int asciiHash(String key){
        return asciiHashRecursive(key, 0);
    }

    public int asciiHashRecursive(String key, int total) {
        if(key.length() == 0) {
            return total;
        } else {
            char letter = key.charAt(0);
            String sub = key.substring(1);
            total += Math.abs(letter);
            return asciiHashRecursive(sub, total);
        }
    }
    ...
    public int primeHash(String key){
        return primeHashRecursive(key, 9);
    }

    public int primeHashRecursive(String key, int total) {
        if(key.length() == 0) {
            return total;
        } else {
            char letter = key.charAt(0);
            String sub = key.substring(1);
            total += Math.abs(letter * 17);
            return primeHashRecursive(sub, total);
        }
    }
    /**TODO - Complete This Function
